...
const Sidebar = () => {
    ...
    return (
        <Fragment>
            <button className="flex items-end w-max ml-auto bg-main p-3 rounded-md text-secondary transition-all hover:bg-primary" onClick={() => setOpen(true)}>
                <FaFilter />
            </button>
            <Drawer
                open={open}
                onClose={() => setOpen(false)}
                className="!bg-primary overflow-auto"
            >
                <div className="text-left pb-12">
                    <button className="text-gray-400 hover:text-white absolute right-3 top-3" onClick={() => setOpen(false)}>
